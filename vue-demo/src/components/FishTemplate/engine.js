/*
 * @Author: liuxin
 * @Date: 2021-07-21 23:07:15
 * @LastEditTime: 2021-07-23 17:11:46
 * @LastEditors: liuxin
 * @Description:
 */
...
export default class Engine {
  ...
  render(template, data) {
    ...
    while (re1.test(template) || re2.test(template)) {
      ...
      template = template.replace(re1, (s0, s1, s2, s3) => {
        ...
        // tag, attr, children, parent, childrenTemplate
  ...
  scopehtmlParse(node, globalScoped, currentScoped) {
    return node.childrenTemplate.replace(/\{\{(.*?)\}\}/g, (s0, s1) => {
      let props = s1.split(".");
      let val = currentScoped[props[0]] || globalScoped[props[0]];
